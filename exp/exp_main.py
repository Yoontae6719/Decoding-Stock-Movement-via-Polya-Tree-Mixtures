...
class Exp_Main(Exp_Basic):
    ...
    def objective(self, trial):
        try:
            ...
            alpha_fs = trial.suggest_float("alpha", 0.1, 1.2, log=True)
            beta_fs  = trial.suggest_float("beta", 0.1, 1.2, log=True)
            ...
            hidden_dim_expert = trial.suggest_categorical("hidden_dim_expert", [8, 16, 32])
           # initial_temp  = trial.suggest_categorical("initial_temp", [1.0, 2.0])
           # final_temp    = trial.suggest_categorical("final_temp", [0.2, 0.1])
            anneal_epochs = trial.suggest_categorical("anneal_epochs", [10, 20, 30])
            ...
            lradj = trial.suggest_categorical("lradj", ["type1", "type2","cosine"])
         ...
         #   self.args.final_temp    = final_temp
            ...
            self.args.schedule_type = schedule_type
            self.args.lradj = lradj
    
    
            setting = (
                f"trial_{trial.number}_data_{self.args.data}_"
                f"alpha{alpha_fs:.5f}_beta{beta_fs:.5f}_lr{learning_rate:.4f}_lradj_{lradj}_"
                f"depth{max_depth}_mlp{use_gating_mlp}_hdim{hidden_dim_expert}_gdim{gating_mlp_hidden}_"
                f"anneal{anneal_epochs}_st{schedule_type}"
            )
